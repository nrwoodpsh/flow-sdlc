...
구조 정본은 `doc/01.architecture.md`, 앞선 판 진단은 `doc/00.diagnosis/`.

**이 파일은 매 턴 실린다.** 여기 적을 것은 매 턴 알아야 하는 것만이다 —
목록·표는 정본을 가리키고 여기 복제하지 않는다. 앞선 판은 이 파일에 차단 목록을 산문과 표로
...
통과 숫자만 보면 사문화된 검사와 지키는 검사가 구별되지 않는다. 앞선 판이 그렇게 낡았다.
