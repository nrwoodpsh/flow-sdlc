...
갈라진 대가는 문서 표가 한쪽만 싣게 되는 것이다 — '산문 사본' 관계가 셸 몫에 남는다.
그래서 **셸 규칙의 머리말을 고정 형식 주석 블록으로 두고 여기서 긁는다.**

무엇을 생성하나

  README.md         `guard-table`    차단표 전체 — `JSON 정본` 절 + `셸 정본` 절 + 한계
  CLAUDE.md         `guard-summary`  압축판. **이 파일은 매 턴 실린다** — 표를 다 싣지 않는다
                                     (매 턴 실리는 파일에 차단 목록을 산문·표로 두 번
                                      적으면 그 파일이 가드레일 절로 반을 채운다)
...
def limit_rows(gr):
    ...
    한계가 표에서 사라지면 문서가 실제 방어보다 넓게 읽힌다.
    """
    out = []
