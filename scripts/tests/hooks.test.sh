...
#   check-drift-hook.sh  훅이 안 도는 상태를 알아채나  ← 앞선 판은 테스트가 0건이었다 (diag-C 3절)
#
# 돌리는 법:  bash scripts/tests/hooks.test.sh
#
# ── 앞선 판과 달라진 것: 가드 케이스를 손으로 적지 않는다 ────────────────
...
# 앞선 판에서 그대로 이식했다. 규칙을 지워도 이 케이스들은 남아야 한다.
# ══════════════════════════════════════════════════════════
...
# 지금 새 규칙의 과차단 회귀 — 정본을 데이터로 옮기며 늘어난 표면이다
...
# ══════════════════════════════════════════════════════════
# check-drift-hook.sh — **앞선 판은 테스트가 0건이었다** (diag-C 3절).
...
# 경로가 틀리면 훅이 조용히 안 돈다. 그게 앞선 판이 앓던 "약속만 있고 기계는 없다"의 배선 판이다.
# ══════════════════════════════════════════════════════════
head_ "hooks.json 배선"
