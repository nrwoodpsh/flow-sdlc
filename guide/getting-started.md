...
어느 커맨드가 어느 조각을 싣는지는 `flow.topology.json`(`commands.*.loads`)이 정본이고,
왜 그렇게 갈랐나는 [`../doc/02.skills-map.md`](../doc/02.skills-map.md) 가 담는다.
...
- **`gate.legacyExempt` 는 글로브 문자열만 적으면 통과가 아니라 확인(ask)이다.** 기록(`why`·`scope`)이 있어야 조용히 통과하고, 리포 전체급으로 넓으면 무시된다. 항목 형식과 정확한 판정은 `flow.topology.json` `gate` 절의 `legacy-exempt` 가 정본이다 — 여기 옮겨 적지 않는다.
