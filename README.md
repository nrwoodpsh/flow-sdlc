...
구조 정본은 `doc/01.architecture.md` 다. 이 리포가 지금 모습이 된 경위는 `doc/00.diagnosis/` 에 있다.

## 왜 이렇게 만들었나

앞선 판은 세 층이 각각 다른 증상을 냈는데 원인이 하나였다 — **정본이 산문이라 기계가 못 읽는다.**
...
앞선 판 README는 "드리프트 4겹"이라 적었지만 **사람 경로만 4겹이고 AI 경로는 옵트인 CI 1겹**이었다.
...
전부 `.github/workflows/ci.yml`에서 돈다. **`.example`가 아니다** — 앞선 판은 드리프트 CI를 옵트인
