...
# 같은 목록이 셸 `case` 문 + 주석 + 산문 여러 곳에 복제되면 하나 늘리는 비용이
# 그만큼 된다. 그러면 아무도 안 늘리고 미탐이 남는다.
# 목록을 여기 다시 적지 마라 — 적는 순간 정본이 둘이 된다.
#
...
# 한계를 주석에만 적고 목록에서 빠진 명령을 한계로도 안 적으면 문서가 실제 방어보다 넓게 읽힌다.
...
# 세그먼트를 나누는 방식을 바꾸지 마라 — 케이스가 그것에 박혀 있다.
USEP=$'\037'
...
scan() {
  ...
  for (( i=0; i<${#s}; i++ )); do
    ...
    case "$c" in
      ...
      # 인용 밖의 공백만 낱말 경계다. SB·SU 에는 그대로 넣는다
      ' '|$'\t') flush_word ; b+=$c ; u+=$c ;;
      ';'|'&'|'|'|'('|')'|'{'|'}'|'`'|'<'|$'\n') push_seg ;;
      # `>` 도 경계다. 다른 것은 **다음 세그먼트에 표시를 남기는 것**뿐이다.
...
# 커밋 메시지고 플래그가 아니다 — 안 빼면 정상 커밋이 막힌다(케이스가 그걸 못 박았다).
