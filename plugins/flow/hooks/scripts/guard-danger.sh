...
#
# ── 앞선 판과 달라진 것: 차단 목록이 이 파일에 없다 ──────────────────
# **차단 목록의 정본은 `guard-rules.json` 이다.** 이 스크립트는 그걸 읽어 판정만 한다.
# 앞선 판은 같은 목록이 셸 `case` 문 + 주석 + 산문 5곳에 복제되어 하나 늘리는 비용이
# 9곳이었다. 그래서 아무도 안 늘렸고 실측 미탐이 9건 남았다 (diag-C 4절).
# 목록을 여기 다시 적지 마라 — 적는 순간 앞선 판의 병이 돌아온다.
#
...
# 앞선 판은 한계도 주석에 적어 두고 목록에서 빠진 명령은 한계로도 안 적었다.
...
if [ -z "$rl_out" ]; then
  ...
  # 아무 말도 없으면 그 상태가 "안전하다"로 읽힌다 — 앞선 판이 정확히 그렇게 낡았다.
...
# ── 여기부터 아래는 앞선 판에서 **그대로** 이식한 인용 인식 스캐너다 ──────
...
# 세그먼트를 나누는 방식은 앞선 판 그대로다(케이스가 그것에 박혀 있다).
USEP=$'\037'
...
scan() {
  ...
  for (( i=0; i<${#s}; i++ )); do
    ...
    case "$c" in
      ...
      # 인용 밖의 공백만 낱말 경계다. SB·SU 에는 앞선 판처럼 그대로 넣는다
      ' '|$'\t') flush_word ; b+=$c ; u+=$c ;;
      ';'|'&'|'|'|'('|')'|'{'|'}'|'`'|'<'|$'\n') push_seg ;;
      # `>` 도 앞선 판과 똑같이 경계다. 다른 것은 **다음 세그먼트에 표시를 남기는 것**뿐이다.
...
# 커밋 메시지고 플래그가 아니다 — 안 빼면 정상 커밋이 막힌다(앞선 판 케이스가 그걸 못 박았다).
