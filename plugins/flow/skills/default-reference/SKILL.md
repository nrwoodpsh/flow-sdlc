...
Claude Code는 자동 RAG 인덱싱이 없다 — 파일 참조는 명시해야 한다. 커맨드마다 무엇을 늘 로드하고 무엇을 안 읽나를 여기서 정한다. 사용자 `[REFERENCE]`는 기본값에 **추가**된다(덮어쓰지 않는다).
...
**글을 쓰는 커맨드는 `plain-writing`을 쓴다 — 기본값이다.** 안 쓰는 것은 아래뿐이다.
...
- **`/flow:design` 전 확인은 필수다**(`CLAUDE.md`의 `참조 통제`) — 용어를 모르면 설계가 어긋난다.
