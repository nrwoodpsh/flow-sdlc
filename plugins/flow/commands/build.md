...
앞선 판은 `build` 와 `verify unit` 을 갈라 뒀지만 build 루프가 이미 단위 테스트를 돌리고 `5.verify/` 에 적었다. 합친다.
...
**면제는 `gate.exemptions` 가 정본이다** — `spike/` 아래 · **면제로 등록된 경로** · 유닛이 하나도 없는 도입 첫날 · 소스가 아닌 것. 레거시에 신규용 게이트를 그대로 걸면 정상 작업이 전부 막히고, **과차단이면 사람이 훅을 꺼 버린다** — 그러면 그 층이 영구히 없어진다. 면제를 커맨드가 판정하지 않는다.

**막혔을 때 — 이 순서로만 푼다.** 훅을 끄거나 우회하는 것은 어느 칸에도 없다.

| 순서 | 상황 | 무엇을 한다 |
|:--|:--|:--|
| 1 | 이번 작업으로 고쳐야 할 파일이다 | **task 문서 `## 3. File Map` 에 `[Mod] 경로` 를 적는다.** 대부분 여기서 끝난다 |
| 2 | 버릴 코드다 | `spike/` 아래에 쓴다 |
| 3 | 범위가 설계와 달라졌다 | 멈추고 `/flow:design` 으로 돌린다 — File Map 을 늘려 설계 밖으로 번지지 않는다 |
| 4 | **flow 가 관리할 구역이 아니다** | `workflow.config.json` 의 `gate.legacyExempt` 에 등록을 **제안하고 사람 승인을 받는다**. 항목 형식·판정은 `flow.topology.json` 의 `gate.exemptions` 중 `legacy-exempt` 가 정본이다 |

- **면제는 File Map 의 대체물이 아니다.** 4번은 벤더 배포본·생성물·남의 팀 소유처럼 *task 문서를 붙일 뜻이 없는* 코드일 때다. 막혔다는 이유만으로 4번을 고르면 게이트가 이름만 남는다.
- **`workflow.config.json` 은 게이트가 안 막는다**(자기 근거를 잠그지 않기 위해서다). 그래서 면제를 스스로 늘릴 수 있다 — **하지 않는다.** 등록은 사람 승인 뒤이고, 무엇을 왜 여는지(`path`·`why`·`scope`·`until`)를 함께 보여준다.
- 게이트가 `확인`(ask)으로 물어 오면 **그것이 답이 아니라 질문이다.** 기록 없는 면제·만료된 면제가 그렇게 나온다 — 승인으로 넘기지 말고 이유를 채우거나 1번으로 간다.
