...
v1 은 이 둘을 한 커맨드 본문에 붙여 뒀는데 도구·에이전트·병렬 전략이 전혀 다르다. **인자로 가르고 본문 조각도 가른다.**
...
진입 조건의 정본은 `flow.topology.json` 의 `commands.verify.entry` 다. **이 커맨드가 스스로 판정하지 않는다.**

- **기계** — `unit-task-doc`. task 문서가 있어야 무엇을 검증하는지가 정해진다.
- **내용** — `coverage-gap`. 커버리지 감사가 낸 결론을 **`gatekeeper` 에 넘긴다. 반드시 부른다** — 감사한 쪽이 자기 결론을 확인하면 편향된다.

  ```
  gatekeeper 위임 — entry.content 의 coverage-gap
