...
| **기능** (work-unit) | `01.work/{도메인}/{NN.유닛}/0.requirement.md` | `…/1.design.md` | `/flow:spec`→`/flow:build`→… |
...
│   │   └── VERSION              복사 시점의 flow 버전 (setup 업데이트 모드가 비교 — 손대지 않음)
│   ├── 04.theme/                테마 스펙 정본 (/flow:theme 입력 · 없으면 빈 폴더)
