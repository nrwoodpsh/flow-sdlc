...
| `03.templates` | 산출물별 표준 템플릿 | 유닛 사슬 `00.requirement`~`07.summary` · 그 밖 `08.adr`·`09.domain`·`10.explainer`·`11.postmortem`·`12.sop-runbook`·`13.architecture`·`14.integration`·`15.theme` + **`VERSION`** | [03.templates/](03.templates/) — **`doc-verify`의 채점 기준** |
| `04.theme` | **테마 스펙 정본** | `/flow:theme` 입력 (토큰·컴포넌트) | 없으면 빈 폴더 |
| `05.explainer` | 신규 인력용 설명 노트 | `NN.{주제}.md` (템플릿 `10.explainer`) | 없으면 빈 폴더 |

> **`03.templates/VERSION`** — 이 템플릿들이 복사된 시점의 flow 버전. `/flow:setup` 업데이트 모드가 플러그인 버전과 비교해 **바뀐 템플릿을 파일별로 제시**한다(절차 1-A). **손으로 고치지 않는다** — 고치면 동기화 판정이 틀어진다. 일부러 고친 템플릿은 `고친 것` 목록에 적어두면 업데이트에서 건너뛴다.
...
| 등급 | 뜻 | 안 채우면 |
|:--|:--|:--|
| **`[진행 필수]`** | 다음 커맨드가 이 내용을 소비한다 | FAIL · `/flow:run`이 **차단** |
