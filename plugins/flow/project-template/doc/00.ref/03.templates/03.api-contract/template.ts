 ...
 *
 * 범위: 내부 전용 | 서비스 간 | 외부 제공   **[진행 필수]** ← 반드시 적는다 (없으면 `내부 전용`으로 본다)
 *   커밋 준비가 이 값을 읽어 repo 밖 호출부를 사람에게 넘긴다(`code-graph` 의 서비스 경계).
 ...
 * ─────────────────────────────────────────────
 * 등급 표기는 문서 템플릿과 같다 — **[진행 필수]** 는 다음 커맨드가 그 내용을 소비한다는 뜻이다
 * (등급의 뜻은 `doc-verify` 의 `grade` 조각이 정본).
 *
...
// 1) 엔드포인트 — method + path를 상수로 고정   **[진행 필수]**
...
// 4) 에러 레지스트리 — code + HTTP status + message   **[진행 필수]**
