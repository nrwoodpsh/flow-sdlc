...
> `/flow:design` 산출. **구조를 확정**한다 — task 분할·계약은 `/flow:spec`(`2.task`·`3.contract`).
...
디자인 토큰·컴포넌트는 `/flow:theme` 산출(`00.ref/04.theme/`)을 참조한다 — **여기서 색·폰트를 새로 정하지 않는다.**
...
- 요구가 설계 요소로 하나도 안 덮이면 누락이다 — `/flow:spec` 전에 채운다.
