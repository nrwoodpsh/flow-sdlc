...
| **DB — 운영·스테이징** | **MCP** | `/flow:prd {레벨} legacy` — 현행 스키마 파악 | `[인증필요]` `[선택]` | **읽기 전용 계정으로만** | 사람이 스키마를 떠서 `02.db-schema/`에 넣는다 |
| **Context7** | **MCP** | `/flow:design`·`/flow:build` — **쓰는 라이브러리의 최신 문서**를 끌어온다. 낡은 API로 짜는 것을 막는다 | `[자동]` `[선택]` | `npx ctx7 setup --claude` (대화형 OAuth) · 무료 등급 있음 | 학습 시점 기억에 의존 — 버전이 다르면 틀릴 수 있다 |
...
3. 설치되면 `workflow.config.json` 의 `test.browser` 를 `chrome` 으로 둔다

> 확장이 붙으면 Claude Code에 브라우저 도구가 자동으로 나타난다 — 별도 MCP 등록이 필요 없다.

- **둘 다 없으면 `/flow:verify` 의 화면 테스트와 테마 적용이 멈추고 설치를 안내한다.**
- 둘 다 있어도 된다 — **어느 것을 쓸지는 `workflow.config.json` 의 `test.browser` 가 정한다.** 커맨드가 도구를 고르지 않는다.
- `chrome` 모드는 **모달·확인창을 띄우는 조작을 피한다** — 대화가 멈춘다.

### 디자인 — 테마 적용 (`theme-apply`)
...
### 이력 조회 — `/flow:design`·`/flow:prd {레벨} legacy`·`/flow:review`
...
### 최소 구현 — `/flow:build`·`/flow:review`·`/flow:prd {레벨} legacy`
...
| 도구 | 종류 | 무엇을 | 마커 | 없을 때 |
|:---|:---|:---|:---|:---|
| **ponytail** ⭐ | **플러그인** | ① 코드 쓸 때 판단 순서 자동 주입 ② `/ponytail-review` — **삭제 목록** ③ `/ponytail-audit` — 저장소 전체 과설계 지도 | **`[필수]`** `[사용자 설치]` | `code-review` 의 체크리스트·`builder` 규칙으로 대체 — **문장 지침이라 강제력이 약하고 삭제 목록·전체 감사는 못 한다** |
...
>
> 저자 자체 측정치는 **표본이 얇다**(repo 1개 · 시행 한 자리 수 · 작은 모델). 단발 벤치마크도 저자들이 부풀려졌다고 인정한다. **수치를 여기 옮겨 적지 않는다** — 우리 환경에서 재보기 전에는 근거가 아니고, 옮겨 적으면 그 값만 남아 낡는다.

### 코드리뷰 — `/flow:review`

`/flow:review`는 **층을 쌓아** 본다. 각 도구가 다른 종류의 오류를 잡는다.

**층은 이름으로 부른다** — 번호로 부르면 층이 하나 늘 때 밖에서 가리킨 줄이 조용히 어긋난다. 무엇이 무엇을 잡나는 `code-review` 스킬이 정본이다.

| 층 | 도구 | 종류 | 무엇을 잡나 | 마커 | 없을 때 |
|:--|:---|:---|:---|:---|:---|
| `계약` | `contract-gate` | 내장 | 타입·컴파일 | — | — |
| `패턴` | **open-code-review (OCR)** ⭐ | **CLI** | NPE·스레드 안전·XSS·SQLi — **알려진 패턴**. 줄 단위 코멘트 | **`[필수]`** `[자동]` | 룰 기반 검출이 빠지고 LLM 판단만 남는다(오탐 증가) |
| `범위` | **Joern (CPG)** ⭐ | CLI | **어디까지 번지나 · 값이 어디로 흐르나** — 전이 영향·taint | **`[필수]`** `[자동]` | 축소 모드(LSP+grep, 1~2홉) — **흐름 추적은 못 한다** |
| `규약·적정함` | 체크리스트 + **ponytail** | 내장 스킬 + 플러그인 | **우리 규약**(계약·명명·범위·테스트) + **과설계 삭제 목록** | ponytail은 위 절 | 체크리스트만 — 삭제 목록 없음 |
| `취약점` | **claude-security** ⭐ | 플러그인 | 취약점 — **발견을 반박한 뒤 보고**, 패치 제안까지 | **`[필수]`** `[사용자 설치]` | `code-review` 의 체크리스트 + LLM 판단만 (오탐 증가) |
| `설계 의심` | **codex** | 플러그인 | **`deep`일 때만** — 다른 모델이 설계 선택을 의심 | `[선택]` `[사용자 설치]` | **`deep`이 앞 층만 돌고 그렇게 알린다** |
| `반증` | `gatekeeper` | 내장 에이전트 | critical·high **반증** | — | — |
...
| **없어도 된다** | `deep`이 앞 층만 돌고 "codex 없음 — 설계 의심 층 생략"을 리포트에 적는다. 멈추지 않는다 |
| **쓰지 않을 것** | `/codex:rescue`(코드를 고친다 — 쓰기는 `builder`만) · `/codex:review`(`패턴`·`취약점` 층과 중복) |
...
- **`code-review` 의 체크리스트와 역할이 다르다** — 체크리스트는 *우리 규약을 지켰나*(계약·명명·범위), 이건 *위험한 값·패턴이 있나*.
- **발견을 보고 전에 스스로 반박**한다 — 우리 `gatekeeper` 와 같은 사상이라 오탐이 적다.
